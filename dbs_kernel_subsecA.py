        ...
        # =============================================================================
                    ...
                    if save_clus =='yes' or save_clus =='y':
                        ...
                        if len(check_folder) == 0:
                            ...
                            plt.savefig(pruebas + 'Sec_%s_clus/'%(section) +'plt_cluster_num%s_%s_knn%s_area%.2f/'%(clus_num,i,samples_dist,area)+
                                       'cluster%s_%.0f_%.0f_knn_%s_area_%.2f.txt'%(clus_num,ic/0.5,jr/0.5,samples_dist,area), bbox_inches='tight')
                                ...
                                for n_txt in clus_lst:
                                    ...
                                    if len(intersection)> 0 :
                                        ...
                                        np.savetxt(f_check+'/'+
                                                   ...
                                                   header ='ra, dec, l, b, pml, pmb,J, H, Ks,x, y, Aks_mean, dAks_mean, radio("),cluster_ID')
                                        plt.savefig(f_check+'/'+
                                                   'plt_cluster%s_%.0f_%.0f_knn_%s_area_%.2f.txt'%(clus_num,ic/0.5,jr/0.5,samples_dist,area),bbox_inches='tight')
