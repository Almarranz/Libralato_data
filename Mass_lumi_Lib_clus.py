...
path_clus ='/Users/amartinez/Desktop/PhD/Libralato_data/pruebas/'
id_lib = 14996
knn = 20
rad_clus = 50
ra, dec, mura,mudec, H,K = np.loadtxt(path_clus
                                                 + 'clus_%s_%s_%s.txt'%(id_lib,knn,rad_clus),unpack=True)
...
ax.scatter(H-K,K,color ='lime',s=100, label = '%s'%(id_lib),zorder =3)

# ax.scatter(clus['m_hawki_H']-clus['m_hawki_Ks'],clus['m_hawki_Ks'],color = 'fuchsia',alpha=0.7,s =200, zorder =3)
...
Ks_sor = np.arange(min(np.round(np.round(clus['m_hawki_Ks'],0))),max(np.round(np.round(clus['m_hawki_Ks'],0))),.5)
...
# min_col[relleno] = np.min(np.delete(min_col,relleno))
# max_col[relleno] = np.max(np.delete(max_col,relleno))
# ax.plot(min_col,Ks_sor[0:-1])
# ax.plot(max_col,Ks_sor[0:-1])
# 
# ax.patch(Rectangle((min_col[5],Ks_sor[5]),min_col[6]-min_col[5],Ks_sor[6]-Ks_sor[5]))
for j in range(len(Ks_sor)-2):
    ...
    print(min_col[j],Ks_sor[j], max_col[j+1]-min_col[j], Ks_sor[j+1]-Ks_sor[j])
# ax.add_patch(Rectangle((min_col[6],Ks_sor[6]), max_col[6]-min_col[5], Ks_sor[6]-Ks_sor[5],alpha =0.3,edgecolor = None,facecolor = 'gray'))
