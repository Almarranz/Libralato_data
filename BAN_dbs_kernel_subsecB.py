...
# %%
lim_pos_up, lim_pos_down = -214.358, - 214.450 #intersection of the positives slopes lines with y axis,
lim_neg_up, lim_neg_down = 237.63 ,237.28
# %%
...
#                         yg_1 = (lim_pos_up - (ic)*step/np.cos(np.pi/4)) +  m*catal[:,0]
#                         yg_2 = (lim_pos_up - (ic+1)*step/np.cos(np.pi/4)) +  m*catal[:,0]
#                         
...
#                             ax.scatter(catal[:,7],catal[:,8],alpha =0.01)
#                             jr *=0.5
#                             yr_1 = (lim_neg_up - (jr)*step_neg/np.cos(ang)) +  m1*catal[:,0]
#                             # yg_2 = (lim_pos_up - (i+1)*step*np.cos(45*u.deg)) +  m*catal[:,7]
#                             yr_2 = (lim_neg_up - (jr+1)*step_neg/np.cos(ang)) +  m1*catal[:,0]
#                             good = np.where((catal[:,2]<yg_1)&(catal[:,2]>yg_2)
#                                                     & (catal[:,2]<yr_1)&(catal[:,2]>yr_2))
#                             area = step*step_neg*0.05**2/3600
#                             
#                             
#                             ax.scatter(catal[:,0][good],catal[:,2][good],color =strin[np.random.choice(indices)],alpha = 0.1)
#                             
