...
iso_dir_HST = '/Users/amartinez/Desktop/PhD/Libralato_data/nsd_isochrones/HST'
...
iso =  synthetic.IsochronePhot(logAge, AKs, dist, metallicity=metallicity,
                                evo_model=evo_model, atm_func=atm_func,
                                red_law=red_law, filters=filt_list,iso_dir=iso_dir_HST)
...
# mag_test = np.where((f153>14)&(f153<20))
# mag_test = np.where((f153<20))
# f153 = f153[mag_test]
# f127 = f127[mag_test]
# #%

M_clus = 2*1e4*u.Msun
max_stars = len(f153)**2
porcentaje = 0
while  max_stars > len(f153)+0.1*len(f153):
    ...
    max_mass = np.where((clus_ndiff['m_hst_f153m']>min(f153))&(clus_ndiff['m_hst_f153m']<max(f153)))
...
ax.scatter(f127-f153,f153,color ='#ff7f0e',s=100, label = '%s'%(choosen_cluster),zorder =3)

# ax.scatter(clus['m_hawki_H']-clus['m_hawki_Ks'],clus['m_hawki_Ks'],color = 'slategray',alpha=0.7)
ax.scatter(clus_ndiff['m_hst_f127m']-clus_ndiff['m_hst_f153m'],clus_ndiff['m_hst_f153m'],
...
all_color = clus['m_hst_f127m']-clus['m_hst_f153m']
min_col = []
max_col = []
ax.invert_yaxis()
# Ks_sor = np.arange(min(clus['m_hawki_Ks']),max(clus['m_hawki_Ks']),0.5)
Ks_sor = np.arange(min(np.round(np.round(clus['m_hst_f153m'],0))),max(np.round(np.round(clus['m_hst_f153m'],0))),1)
